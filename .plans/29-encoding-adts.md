...
domain X {
  ...
  // Rank decrease — only emitted for constructor fields whose type is itself a Gobra ADT.
  // F must have its own rank$F function (generated by F's domain declaration).
  // Fields of primitive Silver types (Int, Bool, Ref, Seq[T], Map[K,V], etc.) have no
  // rank$ function; no rank-decrease axiom is emitted for them.
